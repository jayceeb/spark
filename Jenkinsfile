...
def prBuilder(){
  def mvnArgs="-Pdeb -U -Dscala-2.11 -Phive -Phive-thriftserver -Phadoop-2.6 -Pyarn -Dgpg.skip=true -Dmaven.javadoc.skip=true"
  stage('PR Build'){
...
def releaseBuilder(){
  def mvnArgs="-Pdeb -U -Dscala-2.11 -Phive -Phive-thriftserver -Phadoop-2.6 -Pyarn -DskipTests -Dgpg.skip=true -Dmaven.javadoc.skip=true"
  stage('Release Clean'){
  ...
  stage('Release Docker Image'){
    def dockerVersion = "csd-1.6.3-${spark_version.split('-')[-1]}"
    sh """
